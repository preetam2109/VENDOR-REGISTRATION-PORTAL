...
@Component({
  selector: 'app-confirm-capaity-of-production-tab',
  ...
  standalone:true,
  templateUrl: './confirm-capaity-of-production-tab.html',
  styleUrl: './confirm-capaity-of-production-tab.css'
})
export class ConfirmCapaityOfProductionTab {
  ...
  this.api.GetCOPDetails(this.vregid,this.SupID).subscribe((res: any) => {
      this.mSCDetailsList = res.map((item: any, index: number) => ({
        ...
        sno: index + 1
          ...
          // If navigated via Update button, show only rejected rows (iscopaccepted === 'N')
      if (sessionStorage.getItem('filterRejected') === 'true') {
        this.mSCDetailsList = this.mSCDetailsList.filter((r: any) => r.iscopaccepted === 'N');
